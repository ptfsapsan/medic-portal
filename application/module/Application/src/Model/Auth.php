...
class Auth extends Base{
   ...
   public function login($params){
      ...
      $authAdapter
         ...
         ->setCredential(Main::encrypt($params['password']));
   ...
   public function forgot($email){
      $password = $this->fetchOne("SELECT password FROM users WHERE email = ?",
         $email);
      $password = Main::decrypt($password);
   ...
   public function registration($params){
      $temp_code = md5(Main::encrypt(substr(md5(rand()), 5, 10)));
      $this->insert('users', [
         'email' => $params['email'],
         'password' => Main::encrypt($params['password']),
         'role' => 'user',
         'temp_code' => $temp_code,
      ]);
   ...
   public function activeRegistration($temp_code){
      ...
      $site_code = 0;
      while($f){
         $site_code = Main::generateDigitCode(6);
      ...
      $this->login([
         'email' => $user['email'],
         'password' => Main::decrypt($user['password']),
      ]);
   }
